...
#### *4.2.7.1.2. Downloading Diagnostic Logs for Triage*

When debugging a connection drop or latency issue for a specific session under UAT or load testing, you can retrieve the full log stream using either the GCP Logs Explorer console or the `gcloud` CLI tool.

### **Approach A: GCP Logs Explorer UI (Interactive Triage)**
Use the following query in the GCP Console Logs Explorer for quick diagnostics. Note that Log Explorer downloads are capped at a maximum of **10,000 entries**.

```sql
-- 1. Primary Log Group (Includes stdout to prevent future query breakage)
(
  logName="projects/YOUR_PROJECT_ID/logs/run.googleapis.com%2Fstdout"
  OR logName="projects/YOUR_PROJECT_ID/logs/run.googleapis.com%2Fstderr"
  OR logName="projects/YOUR_PROJECT_ID/logs/run.googleapis.com%2Frequests"
  OR logName="projects/YOUR_PROJECT_ID/logs/requests"
  OR logName="projects/YOUR_PROJECT_ID/logs/cloudaudit.googleapis.com%2Fdata_access"
  OR logName="projects/YOUR_PROJECT_ID/logs/ces.googleapis.com%2Fresponses"
)

-- 2. Session & Diagnostics Filters (Replace with active session parameters)
AND (
  jsonPayload.adapter_session_id = "YOUR_ADAPTER_SESSION_UUID"
  OR labels.session_id = "YOUR_CES_SESSION_ID"
  OR labels.instanceId = "YOUR_CONTAINER_INSTANCE_ID" -- Optional: to triage co-tenant interference on the same container
  OR (
    httpRequest.userAgent = "GenesysCloud-AudioHook-Client" -- LB entry check
    AND resource.type = "http_load_balancer"
  )
)

-- 3. Dynamic Time Window (Target load test window + 1-min buffer)
AND timestamp >= "START_TIME_ISO_8601"
AND timestamp <= "END_TIME_ISO_8601"

-- 4. Clean Exclusions
AND NOT protoPayload.serviceName = "storage.googleapis.com"
AND NOT protoPayload.serviceName = "secretmanager.googleapis.com"
AND NOT protoPayload.methodName = "google.cloud.ces.v1beta.AgentService.GetConversation"
```

### **Approach B: `gcloud` CLI (Bulk Log Export)**
For large load tests generating more than 10,000 logs, use the `gcloud` CLI to stream all matches directly to a local JSON file:
...
AND (
  ...
  OR labels.instanceId = "YOUR_CONTAINER_INSTANCE_ID" -- Optional: to triage co-tenant interference on the same container
  OR (
    httpRequest.userAgent = "GenesysCloud-AudioHook-Client"
    AND resource.type = "http_load_balancer"
  )
)
AND timestamp >= "START_TIME_ISO_8601"
...
--limit=100000 \
--format="json" > session_triage_logs.json
```

**Parameters description:**
*   `YOUR_PROJECT_ID`: The Google Cloud project ID where the adapter is deployed.
*   `YOUR_ADAPTER_SESSION_UUID`: The UUID generated by the adapter when the call connects (`log_type="connection_start"`).
*   `YOUR_CES_SESSION_ID`: The session ID representing the Google CES agent session.
*   `YOUR_CONTAINER_INSTANCE_ID`: (Optional) The Cloud Run container instance ID that served the call. Querying this shows if another session running on the same container instance interfered with the event loop.
*   `START_TIME_ISO_8601` / `END_TIME_ISO_8601`: The time range of the load test or session connection. We recommend adding a **1-minute safety padding** before the start time and after the end time (e.g., `2026-07-09T15:26:00Z`).
